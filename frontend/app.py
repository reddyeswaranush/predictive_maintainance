...
DEFAULT_DEMO_USERS = {
    os.getenv("FACTORYOPS_ADMIN_USER", "admin")
    .strip()
    .lower(): {
...
def inject_styles() -> None:
    st.markdown(
        ...
        /* Login panel: keyed Streamlit container keeps the form, tabs, and fields
           inside one real styled surface. */
        .st-key-login_panel {
        ...
        .st-key-login_panel [data-baseweb='tab-list'] {
            gap: 0.45rem;
            border-bottom: 1px solid var(--border);
        }

        .st-key-login_panel [data-baseweb='tab'] {
        ...
        .st-key-login_panel [data-baseweb='tab'] p,
        .st-key-login_panel [data-baseweb='tab'] span {
            color: var(--text) !important;
            font-weight: 600 !important;
        }

        .st-key-login_panel [data-baseweb='tab']:hover {
            background: rgba(229, 214, 195, 0.55);
        }

        .st-key-login_panel [aria-selected='true'] {
            color: var(--text) !important;
        ...
        .st-key-login_panel [aria-selected='true'] p,
        .st-key-login_panel [aria-selected='true'] span {
            color: var(--text) !important;
            font-weight: 700 !important;
        }

        .st-key-login_panel .stTextInput input,
        .st-key-login_panel .stSelectbox > div,
        .st-key-login_panel div[data-baseweb='select'] > div:first-child {
            min-height: 2.75rem;
        }
        ...
        /* Fix Sign In / Sign Up tab text visibility */
...
def create_account(
    name: str, username: str, password: str, role: str
) -> tuple[bool, str]:
...
def render_login_page() -> None:
    ...
    with middle:
        # A keyed Streamlit container is used instead of an HTML <div>.
        # Streamlit components are rendered outside raw HTML wrappers, so this
        # keeps the tabs and forms inside the same visible login card.
        with st.container(key="login_panel"):
            ...
            with sign_up_tab:
                with st.form("factoryops_signup", clear_on_submit=False):
                    ...
                    role = st.selectbox(
                        "Role",
                        ["Operator", "Maintenance", "System Administrator"],
                    )
...
def get_records(resource: str) -> list[dict[str, Any]]:
    try:
        response = requests.get(
            api_url(f"{resource}/"), timeout=REQUEST_TIMEOUT_SECONDS
        )
...
def send_record(
    method: str, resource: str, payload: dict[str, Any], record_id: int | None = None
) -> tuple[bool, str]:
...
def state_badge(state: str) -> str:
    ...
    css = (
        "pill-good"
        if state
        in {"running", "active", "normal", "healthy", "completed", "closed", "resolved"}
        else (
            "pill-bad"
...
def build_machine_view(
    machines: pd.DataFrame, telemetry: pd.DataFrame, predictions: pd.DataFrame
) -> pd.DataFrame:
    ...
    risk_columns = [
        column for column in machine_view if column.startswith("failure_probability")
    ]
    machine_view["failure_probability_display"] = (
        machine_view[risk_columns].bfill(axis=1).iloc[:, 0].fillna(0.0)
        if risk_columns
        else 0.0
    )
    health_columns = [
        column for column in machine_view if column.startswith("health_score")
    ]
    machine_view["health_score_display"] = (
        machine_view[health_columns].bfill(axis=1).iloc[:, 0].fillna(0.0)
        if health_columns
        else 0.0
    )
...
def render_search_filter_section(
    ...
    locations = (
        sorted(machine_view["location"].dropna().astype(str).unique())
        if "location" in machine_view
        else []
    )
    statuses = (
        sorted(machine_view["status"].dropna().astype(str).unique())
        if "status" in machine_view
        else []
    )
    with left:
        query = st.text_input(
            "Search machines",
            placeholder="Search by machine name, department, or location",
        )
    with middle:
    ...
    with right:
        selected_statuses = st.multiselect(
            "Machine status", statuses, placeholder="All statuses"
        )
    st.markdown("</div>", unsafe_allow_html=True)
...
def apply_filters(
    machine_view: pd.DataFrame, locations: list[str], statuses: list[str], query: str
) -> pd.DataFrame:
    ...
    if query:
        searchable = [
            column
            for column in ["machine_name", "department", "location"]
...
def render_command_center(
    ...
    critical = int(
        machine_view.get("condition", pd.Series(dtype=str)).eq("critical").sum()
    )
    warning = int(
        machine_view.get("condition", pd.Series(dtype=str)).eq("warning").sum()
    )
    average_health = (
        number_column(machine_view, "health_score_display").replace(0, pd.NA).mean()
    )
    ...
    with metrics[0]:
        render_metric_card(
            "Fleet availability",
            f"{running}/{total}",
            "Machines currently marked running",
        )
    with metrics[1]:
        render_metric_card(
            "Average health", f"{average_health:.0f}%", "Latest reported health score"
        )
    with metrics[2]:
        render_metric_card(
            "Critical attention", str(critical), "High-risk or overheating assets"
        )
    with metrics[3]:
        render_metric_card(
            "Watch list", str(warning), "Assets outside normal thresholds"
        )
    with metrics[4]:
        open_incidents = int(
            incidents.get("status", pd.Series(dtype=str))
            .astype(str)
            .str.lower()
        ...
        render_metric_card(
            "Open incidents",
            str(open_incidents),
            "Open and in-progress incident records",
        )

    left, right = st.columns([1.35, 1], gap="large")
    with left:
        st.markdown("#### Condition by machine")
        display = machine_view.sort_values(
            ["condition", "failure_probability_display"], ascending=[True, False]
        ).head(12)
        for start in range(0, len(display), 3):
            columns = st.columns(3)
            for column, (_, machine) in zip(
                columns, display.iloc[start : start + 3].iterrows()
            ):
                with column:
    ...
    with right:
        ...
        if machine_view.empty:
            ...
            chart_data = machine_view.sort_values(
                "failure_probability_display", ascending=False
            ).head(10)
            condition_colours = {
                "normal": "#6c8751",
                "warning": "#d39b52",
            ...
            figure = go.Figure(
                go.Bar(
                    ...
                    orientation="h",
                    marker_color=[
                        condition_colours.get(condition, "#b65f3c")
                        for condition in chart_data["condition"]
                    ],
                    text=[
                        f"{probability:.0%} · {condition.title()}"
                        for probability, condition in zip(
                            chart_data["failure_probability_display"],
            ...
            radar_layout.update(
                ...
                xaxis={
                    ...
                    "range": [
                        ...
                        max(
                            0.1,
                            float(chart_data["failure_probability_display"].max())
                            + 0.15,
                        ),
                    ],
                },
    ...
    with queue_left:
        ...
        risk_table = format_table(
            machine_view.sort_values(
                "failure_probability_display", ascending=False
            ).head(8),
            columns,
        ).copy()
        if "failure_probability_display" in risk_table:
            risk_table["failure_probability_display"] = risk_table[
                "failure_probability_display"
            ].map("{:.1%}".format)
        if "temperature" in risk_table:
            risk_table["temperature"] = risk_table["temperature"].map(
                "{:.1f} °C".format
            )
    ...
    with queue_right:
        ...
        if maintenance.empty:
            ...
            if not machine_view.empty and {"machine_id", "machine_name"}.issubset(
                machine_view.columns
            ):
            ...
            st.dataframe(
                format_table(
                    workload, ["Machine", "Task", "Technician", "Status", "Cost"]
                ),
    ...
    if not telemetry.empty:
        ...
        columns = [
            column
            for column in ["temperature", "vibration", "power", "rpm"]
            if column in snapshot
        ]
        if columns:
            summary = (
                snapshot[columns].mean(numeric_only=True).rename("Average").to_frame()
            )
...
def render_fleet_explorer(machine_view: pd.DataFrame) -> None:
    ...
    with table:
        ...
        export["failure_probability_display"] = export[
            "failure_probability_display"
        ].map("{:.1%}".format)
        ...
        with st.expander("Update a machine status"):
            ...
            with st.form("update_machine_status"):
                new_status = st.selectbox(
                    ...
                    status_options,
                    index=(
                        status_options.index(selected.get("status", "Running"))
                ...
                if st.form_submit_button("Save status"):
                    payload = {
                        ...
                        for key in ["machine_name", "department", "location"]
                    }
                    payload["status"] = new_status
                    ok, message = send_record(
                        "PUT", "machines", payload, int(selected_id)
                    )
...
def render_telemetry_lab(telemetry: pd.DataFrame, machines: pd.DataFrame) -> None:
    ...
    machine_names = (
        ...
        and {"machine_id", "machine_name"}.issubset(machines.columns)
        else pd.DataFrame(columns=["machine_id", "machine_name"])
    )
    data = telemetry.merge(machine_names, on="machine_id", how="left")
    data["machine_label"] = data["machine_name"].fillna(
        "Unknown machine #" + data["machine_id"].astype(str)
    )
    selected_machine = st.selectbox(
        "Inspect machine",
        sorted(data["machine_id"].dropna().unique()),
        format_func=lambda identifier: data.loc[
            data["machine_id"].eq(identifier), "machine_label"
        ].iloc[0],
    )
    selected = (
    ...
    for column, label, suffix in [
        ...
        with metrics[
            ["temperature", "vibration", "rpm", "power", "oil_level"].index(column)
        ]:
    ...
    chosen_signals = st.multiselect(
        "Signals", signal_columns, default=signal_columns[:3]
    )
    if chosen_signals:
        figure = go.Figure()
        ...
        st.caption(
            "The backend does not yet store timestamps, so readings are shown in capture order."
        )

    st.markdown("#### Recent raw readings")
    display_selected = (
        selected.sort_values("id", ascending=False).copy()
        if "id" in selected
        else selected.copy()
    )
...
def render_prediction_incident_page(
    predictions: pd.DataFrame, incidents: pd.DataFrame, machines: pd.DataFrame
) -> None:
    machine_names = (
    ...
    tab_predictions, tab_incidents = st.tabs(
        ["Failure prediction prioritization", "Incident command board"]
    )
    with tab_predictions:
        if predictions.empty:
            ...
            with right:
                ...
                urgent = (
                    view.sort_values(
                        ["failure_probability", "predicted_days"],
                        ascending=[False, True],
                    )
                    .head(8)
                    .copy()
                )
                urgent["failure_probability"] = urgent["failure_probability"].map(
                    "{:.1%}".format
                )
            ...
            for column, status in zip(columns, status_order):
                with column:
                    ...
                    for _, incident in cards.iterrows():
                        ...
                        machine_label = (
                            incident.get("machine_name")
                            or f"Machine #{incident.get('machine_id')}"
                        )
                        st.markdown(
...
def render_maintenance_inventory_page(
    maintenance: pd.DataFrame, inventory: pd.DataFrame, machines: pd.DataFrame
) -> None:
    machine_names = (
        machines[["machine_id", "machine_name"]]
        if not machines.empty
    ...
    tab_maintenance, tab_inventory = st.tabs(
        ["Maintenance planning", "Spare-parts readiness"]
    )
    ...
    with tab_inventory:
        if inventory.empty:
            ...
            with left:
                ...
                figure.update_layout(
                    **chart_layout(400), yaxis={"categoryorder": "total ascending"}
                )
                st.plotly_chart(
                    figure, use_container_width=True, config={"displayModeBar": False}
                )
...
def render_data_management(machines: pd.DataFrame, notifications: pd.DataFrame) -> None:
    st.caption(
        "Submit records directly through the FastAPI API. Required fields match the current backend schemas."
    )
    machine_options = (
        machines["machine_id"].tolist() if "machine_id" in machines else []
    )
    tabs = st.tabs(
        [
            "Machine",
        ...
        if not machine_options:
            ...
            with st.form("create_telemetry", clear_on_submit=True):
                ...
                if st.form_submit_button("Record telemetry"):
                    ok, message = send_record(
                        "POST", "telemetry", {"machine_id": machine_id, **values}
                    )
                    (st.success if ok else st.error)(message)
    with tabs[2]:
        if machine_options:
    ...
    with tabs[5]:
        with st.form("create_notification", clear_on_submit=True):
            ...
            left, right = st.columns(2)
            notification_type = left.selectbox(
                "Type", ["Info", "Alert", "Warning", "Critical"]
            )
        ...
        if not notifications.empty:
            ...
            display_notifications = notifications.sort_values(
                "id", ascending=False
            ).rename(
                columns={
                    "title": "Title",
...
def machine_name_for(machine_id: int, machines: pd.DataFrame) -> str:
    matching = (
        machines[machines["machine_id"].eq(machine_id)]
        if "machine_id" in machines
        else pd.DataFrame()
    )
    return (
        f"#{machine_id} · {matching.iloc[0]['machine_name']}"
...
def main() -> None:
    ...
    if not health:
        page_header(
            "Connection required",
            "Start the FastAPI backend to unlock live fleet operations and data entry.",
            health,
        )
    ...
    collections = {
        resource: as_frame(get_records(resource)) for resource in RESOURCE_LABELS
    }
    ...
    copy = {
        "Command Center": (
            "Command Center",
            "Prioritize operational attention from fleet condition, health, risk, and work queues.",
        ),
        "Fleet Explorer": (
            "Fleet Explorer",
            "Search the registered assets and update operating state from one cleaner workspace.",
        ),
        "Telemetry Lab": (
            "Telemetry Lab",
            "Inspect recorded machine signals, compare conditions, and export the data you need.",
        ),
        "Predictions & Incidents": (
            "Predictions & Incidents",
            "Turn model outputs and reported issues into an action-oriented priority queue.",
        ),
        "Maintenance & Inventory": (
            "Maintenance & Inventory",
            "Plan work, monitor spend, and verify whether required parts are available.",
        ),
        "Data Management": (
            "Data Management",
            "Create the operational records that power the dashboard through the FastAPI service.",
        ),
    }
